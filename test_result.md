...
agent_communication:
  ...
  - agent: "main"
    message: "Itération 14 : photos IA par recette (dish_images.py, gen_dish_images.py, static/dishes/*.jpg servis sur /api/dishes ; recipe.image relatif '/api/dishes/<key>.jpg', frontend dishUrl()). pick_image → IA si dispo sinon stock_image. recipe_name renomme les recettes fixes selon l'aliment servi ; _ensure_metrics migre image+nom. 7 images manquantes (budget clé) → fallback stock. Tests backend_test (image http ou /api/dishes/) et test_iteration13 (/api/dishes accepté) mis à jour ; 140 tests verts."
