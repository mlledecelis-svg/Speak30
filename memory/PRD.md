...
## Itération 14 — photos de plats générées par IA (livrée)
- `backend/dish_images.py` + `backend/gen_dish_images.py` : une photo Gemini Nano Banana (clé universelle Emergent) par recette fixe, par famille de protéine pour les recettes adaptatives (`{id}__{famille}`), par petit-déjeuner et pour la collation. Style : assiette vue de dessus, lumière naturelle, fond crème. Fichiers JPEG dans `backend/static/dishes/` servis via `/api/dishes/…` (StaticFiles) ; frontend `dishUrl()` (program-store) préfixe l'URL backend.
- `engine.pick_image` : photo IA si présente, sinon photo de banque cohérente (`stock_image`). 222/229 images générées ; 7 manquantes (budget journalier de la clé atteint) : relancer `python gen_dish_images.py` après recharge.
- Noms de recettes fixes suivent l'aliment servi dès la génération (« Cabillaud… » → « Merlu… ») ; migration des programmes existants (image + nom) à l'ouverture.
